...
void eval(char *cmdline) 
{
    ...
    char buf[MAXLINE];     // 被修改的命令行
    int bg;                // run in bg or in fg
    pid_t pid;
    ...
    if(builtin_cmd(argv) == 0)
    {
        ...
        sigprocmask(SIG_BLOCK, &mask_one, &prev_one); // 阻塞SIGCHILD
        if((pid = Fork()) == 0)
        {
            setpgid(0, 0); // change process groups
            sigprocmask(SIG_SETMASK, &prev_one, NULL);
            if(execve(argv[0], argv, environ) < 0)
            {
                char msg[100];
                sprintf(msg, "%s: Command not found.", argv[0]);
                app_error(msg);
            }
        } 
